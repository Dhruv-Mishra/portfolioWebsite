...
- The VM has enough disk space and outbound access for the initial Hugging Face model download. Deployment creates `/var/cache/portfolio/pocket-tts`, applies container ownership, bind-mounts it, removes stale TTS cache variables, and atomically persists the GitHub `HF_TOKEN` plus service-specific cache paths. Do not configure these manually. After one successful warmup, offline operation may use `HF_HUB_OFFLINE=1` or `LOCAL_TTS_OFFLINE=1`.
