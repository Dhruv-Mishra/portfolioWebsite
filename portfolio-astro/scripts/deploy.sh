...
#
#   DESCRIPTION: Production deployment for an Astro Node website.
#                Supports two modes:
#
...
#                   Full git pull + npm ci + Astro build on the VM. Kept as a
...
#                --skip-build        [legacy]   Skip npm ci + Astro build
...
stage_release() {
    ...
    if [[ -n "${current_target}" ]] && [[ -n "${target_real}" ]] && [[ "${current_target}" == "${target_real}" ]]; then
        if [[ ! -f "${target}/dist/entry.mjs" ]] && [[ ! -f "${target}/server.js" ]]; then
            log ERROR "Release ${RELEASE_SHA} is already active but does not contain dist/entry.mjs or server.js"
    ...
    [[ -d "${staging}/dist/client" ]] && chmod -R o+rX "${staging}/dist/client" 2>/dev/null || true
...
stage_image_release() {
    ...
    docker cp "${extract_container}:/app/dist/client" "${staging}/dist/" \
        || { log ERROR "Could not extract dist/client from image"; exit 1; }
...
META
    ...
    chown -R "${SERVICE_USER}:${SERVICE_USER}" "${staging}"
    chmod o+x "${DEPLOY_ROOT}" "${DEPLOY_RELEASES_DIR}" "${staging}" 2>/dev/null || true
    chmod -R o+rX "${staging}/dist/client" 2>/dev/null || true
...
ExecStart=${node_bin} dist/entry.mjs
Restart=on-failure
RestartSec=5
TimeoutStartSec=60
TimeoutStopSec=15
...
prepare_docker_systemd_unit() {
    ...
    cat > "${STAGED_SYSTEMD_UNIT}" << UNIT
[Unit]
Description=Astro Node Docker container — ${DOMAIN}
...
artifact_deploy() {
    ...
    [[ -f "${RELEASE_DIR}/dist/entry.mjs" ]] || { log ERROR "dist/entry.mjs missing in release dir"; exit 1; }
...
rollback_deploy() {
    ...
    if [[ -n "${target_image}" ]]; then
        ...
        [[ -f "${RELEASE_DIR}/dist/entry.mjs" ]] || [[ -f "${RELEASE_DIR}/server.js" ]] || {
            log ERROR "Rollback release is neither an image release nor an artifact release with dist/entry.mjs/server.js: ${RELEASE_DIR}"
            exit 1
        }
        prepare_systemd_unit
    fi
...
build_project() {
    ...
    log STEP "Building Astro Node app..."
    cd "${PROJECT_ROOT}"
    ...
    [[ -f "${PROJECT_ROOT}/dist/entry.mjs" ]] \
        || { log ERROR "Astro server output not found"; exit 1; }
    log SUCCESS "Build complete"
}

prepare_legacy_standalone() {
    if [[ "${SKIP_BUILD}" == "true" ]]; then return 0; fi
    log STEP "Preparing Astro dist bundle (legacy mode)..."
    mkdir -p "${LEGACY_STANDALONE_DIR}"
    rm -rf "${LEGACY_STANDALONE_DIR}/dist" "${LEGACY_STANDALONE_DIR}/scripts"
    cp -r "${PROJECT_ROOT}/dist" "${LEGACY_STANDALONE_DIR}/dist"
    mkdir -p "${LEGACY_STANDALONE_DIR}/scripts"
    cp "${PROJECT_ROOT}/scripts/kitten-tts-worker.py" "${LEGACY_STANDALONE_DIR}/scripts/kitten-tts-worker.py"
    if [[ -f "${LEGACY_ENV_FILE}" ]]; then
        cp "${LEGACY_ENV_FILE}" "${LEGACY_STANDALONE_DIR}/.env.local"
    fi
    chown -R "${SERVICE_USER}:${SERVICE_USER}" "${LEGACY_STANDALONE_DIR}"
    local dir="${LEGACY_STANDALONE_DIR}"
    while [[ "${dir}" != "/" ]]; do
        chmod o+x "${dir}" 2>/dev/null || true
        dir="$(dirname "${dir}")"
    done
    chmod -R o+rX "${LEGACY_STANDALONE_DIR}/dist/client" 2>/dev/null || true
    log SUCCESS "Legacy Astro bundle prepared"
}
