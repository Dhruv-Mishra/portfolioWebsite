...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (limited) return Response.json({ suggestions: [] }, { status: 429, headers: { 'Retry-After': String(retryAfter) } });

    const contentLength = Number(request.headers.get('content-length'));
    if (!Number.isFinite(contentLength)) return Response.json({ suggestions: [] }, { status: 411 });
    if (contentLength > MAX_SUGGESTIONS_BODY_BYTES) return Response.json({ suggestions: [] }, { status: 413 });
    ...
    const messages: { role: string; content: string }[] = rawMessages
      .filter((message): message is { role: 'user' | 'assistant'; content: unknown } =>
        typeof message === 'object' && message !== null && isClientChatMessage(message as { role?: unknown; content?: unknown }))
      .map((message) => ({ role: message.role, content: String(message.content) }));

    const context = messages.map((message) => ({ role: message.role, content: String(message.content).slice(0, 300) })).slice(-4);
    ...
    if (!primary) return Response.json({ suggestions: [] });

    const suggestions = await callSuggestionsProvider(primary, context, recentUserMessage, recentAssistantMessage)
      .catch(async () => (fallback ? callSuggestionsProvider(fallback, context, recentUserMessage, recentAssistantMessage).catch(() => []) : []));
...
async function callSuggestionsProvider(
  ...
  try {
    ...
    if (suggestions.length !== 2) throw new Error('Suggestions provider returned invalid output');
    return suggestions;
  } catch {
    clearTimeout(timeout);
