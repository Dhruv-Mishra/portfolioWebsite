...
- Server custom uses Pocket TTS 2.1 with `public/sounds/voice/TTSReference.mp3`, one resident worker, and one active inference. It runs CPU-only Torch 2.10 and persistently caches model data at `/var/cache/<service>/pocket-tts`, including the derived `custom-dhruv.safetensors` voice state. First deployment downloads model weights and derives voice state without int8 mode.
