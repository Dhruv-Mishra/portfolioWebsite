...
Staging image deploys currently publish a single `linux/arm64` image for the Oracle ARM VMs. This avoids multi-platform Next.js chunk drift where HTML from one origin can reference `/_next/static` files generated by a different platform build. The workflow gates VM architecture before deploy and verifies the HTML-referenced static asset graph through both local nginx and Cloudflare.
