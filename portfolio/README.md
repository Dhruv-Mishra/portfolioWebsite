...
Staging and production image deploys publish both `linux/amd64` and `linux/arm64` images. Deploy builds run `next build --webpack` once inside the BuildKit graph, reuse environment-scoped GitHub Actions layer caches, and verify the resulting static graph through local nginx and the public edge. The workflows no longer run a second host-side Next build or upload an unused standalone artifact while image mode is authoritative.

Promotion is branch-based. A genuinely new `dev/lkg` staging promotion increments the minor version from the authoritative `portfolio/package.json`, updates the npm lockfile, creates one release commit, and atomically advances both `dev/lkg` and `deployed/staging`. A retry of the same release does not bump again; it only redispatches the immutable staging SHA. Production then fast-forwards to that exact staged release without changing its version. Bot branch updates explicitly dispatch deploy workflows because token-authored pushes do not recursively trigger them; direct human pushes still trigger the matching deploy workflow.

Both environments preflight every VM before any mutation. The local TTS host deploys and passes service, nginx, asset, synthesis, SHA, and package-version checks as a canary; only then do the two remote hosts deploy in parallel with fail-fast and per-host automatic rollback. Production rollback remains serial.

Node 22, npm 10.9.8, and TypeScript 5 remain the production toolchain. Bun or Yarn would replace the authoritative npm lockfile plus npm-specific lifecycle, CI, and Docker paths without a measured benefit here. TypeScript 7 is not adopted because the installed Next.js ESLint stack currently declares TypeScript support below 6.1; an experimental compiler is not suitable for this release path.
